...
export class MqttMessageValidator {
    static normalizeIncomingMessage(topic: string, rawPayload: Buffer | string, packet: MqttPacketMetadata): MqttMessage {
    ...
    private static validateQos(qos: number): 0 | 1 | 2 {
        if (qos !== 0 && qos !== 1 && qos !== 2) {
            throw new BadRequestException('Qos MQTT inválido. Valores permitidos 0, 1 ou 2.');
        }
