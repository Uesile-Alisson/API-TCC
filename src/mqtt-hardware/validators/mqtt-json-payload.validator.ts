...
export class MqttJsonPayloadValidator {
  static parseToObject(rawPayload: Buffer | string): Record<string, unknown> {
    const payloadAsString = Buffer.isBuffer(rawPayload)
    ...
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          'Payload MQTT deve ser um formato JSON válido.',
        );
      }
      ...
      throw new BadRequestException(
        'Payload MQTT inválido. A mensagem deve estar em formato JSON.',
      );
    }
  }

  static toRawString(rawPayload: Buffer | string): string {
    return Buffer.isBuffer(rawPayload)
      ? rawPayload.toString('utf8')
      : rawPayload;
  }
}
